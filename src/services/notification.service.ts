...
export class NotificationService {
  ...
  static async createOrderNotification(orderId: string, customerName: string, total: number) {
    try {
      await createNotification(
        "ORDER",
        "New Order Received",
        `New order from ${customerName} - Total: ৳${total.toFixed(2)}`,
        orderId
      );
    } catch (error) {
  ...
  static async checkLowStockAlerts(threshold: number = 10) {
    try {
      await dbConnect();
      ...
      for (const product of lowStockProducts) {
        await createNotification(
          "LOW_STOCK",
          "Low Stock Alert",
          `${product.title} is running low (${product.stock} left)`,
          String(product._id)
        );
      }
  ...
  static async triggerStockAlerts(threshold: number = 10) {
