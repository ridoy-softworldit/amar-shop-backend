...
export class InventoryService {
     ...
     */
    static async addStock(productId, quantity, type, performedBy, reason, reference) {
        const session = await mongoose.startSession();
        try {
            await session.withTransaction(async () => {
                // Update product stock atomically
                const product = await Product.findByIdAndUpdate(productId, { $inc: { stock: quantity } }, { new: true, session });
                if (!product) {
     ...
     */
    static async removeStock(productId, quantity, type, performedBy, reason, reference) {
        const session = await mongoose.startSession();
        try {
            await session.withTransaction(async () => {
                ...
                }, { $inc: { stock: -quantity } }, { new: true, session });
                if (!product) {
