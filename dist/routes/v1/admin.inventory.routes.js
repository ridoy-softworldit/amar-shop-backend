...
router.post("/products/:id/stock/add", requireAdmin, async (req, res, next) => {
    try {
        ...
        try {
            ...
            if (product) {
                if (currentStock <= 0) {
                    await createNotification("OUT_OF_STOCK", "Out of Stock Alert", `${product.title} is now out of stock`, id);
                }
                else if (currentStock <= 10) {
                    await createNotification("LOW_STOCK", "Low Stock Alert", `${product.title} is running low (${currentStock} left)`, id);
                }
            }
        }
...
router.get("/stock/out-of-stock", requireAdmin, async (req, res, next) => {
    try {
        await dbConnect();
        const { Product } = await import("../../models/Product.js");
        const products = await Product.find({ stock: { $lte: 0 }, status: "ACTIVE" }, "_id title slug stock price categorySlug subcategorySlug brand").lean().exec();
        res.json({
            ok: true,
...
router.get("/stock/low-stock", requireAdmin, async (req, res, next) => {
    try {
        ...
        const products = await Product.find({ stock: { $gt: 0, $lte: threshold }, status: "ACTIVE" }, "_id title slug stock price categorySlug subcategorySlug brand").lean().exec();
        res.json({
            ok: true,
            data: products.map(p => ({ ...p, _id: String(p._id) }))
        });
