...
router.patch("/notifications/:id/read", requireAdmin, async (req, res, next) => {
    try {
        ...
        const notification = await Notification.findByIdAndUpdate(id, { isRead: true, readAt: new Date() }, { new: true }).lean();
        if (!notification) {
...
router.patch("/notifications/:id/unread", requireAdmin, async (req, res, next) => {
    try {
        ...
        const notification = await Notification.findByIdAndUpdate(id, { isRead: false, $unset: { readAt: 1 } }, { new: true }).lean();
        if (!notification) {
            return res.status(404).json({ ok: false, code: "NOT_FOUND" });
        }
        res.json({
