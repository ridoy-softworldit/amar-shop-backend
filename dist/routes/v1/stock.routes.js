...
router.patch("/products/:id/stock", async (req, res, next) => {
    try {
        ...
        if (typeof payload.set === "number") {
            const updated = await Product.findByIdAndUpdate(id, { $set: { stock: payload.set } }, { new: true, runValidators: true })
                .lean()
                .exec();
            if (!updated)
        ...
        const updated = await Product.findOneAndUpdate(query, { $inc: { stock: delta } }, { new: true, runValidators: true })
            .lean()
            .exec();
        if (!updated) {
